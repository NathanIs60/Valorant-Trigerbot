...
class UltraStableTriggerBot:
    def __init__(self):
        ...
        self.tolerance = tk.DoubleVar(value=0.8)  # 80% - more restrictive
        self.scan_interval = tk.DoubleVar(value=100.0)  # 100ms for more stability
        self.click_cooldown = tk.DoubleVar(value=500.0)  # 500ms for safety
    ...
    def toggle_detection(self):
        ...
        self.log(f"Toggle detection called - New state: {'ACTIVE' if self.detection_active else 'INACTIVE'}")
    ...
    def check_thread_status(self):
        """Debug method to check if detection thread is still running"""
        if self.detection_active and self.detection_thread:
            if self.detection_thread.is_alive():
                self.log("Debug: Detection thread is alive and running")
                # Schedule next check
                self.root.after(5000, self.check_thread_status)
            else:
                self.log("ERROR: Detection thread died unexpectedly! Attempting restart...")
                # Try to restart the thread
                if self.detection_active and self.running:
                    self.detection_thread = threading.Thread(target=self.ultra_stable_loop, daemon=True)
                    self.detection_thread.start()
                    self.root.after(1000, self.check_thread_status)
        elif self.detection_active:
            self.log("ERROR: Detection is active but no thread exists! Attempting restart...")
            if self.running:
                self.detection_thread = threading.Thread(target=self.ultra_stable_loop, daemon=True)
                self.detection_thread.start()
                self.root.after(1000, self.check_thread_status)
    ...
    def ultra_stable_loop(self):
        ...
        while self.running and self.detection_active and not self.force_stop:
            try:
                scan_start = time.time()
                scan_count += 1
                
                # Debug: Log every 1000 scans to verify we're still running
                if scan_count % 1000 == 1:
                    self.log(f"Ultra Stable Loop running - Scan #{scan_count}")
                ...
                try:
                    ...
                    # Require multiple pixel matches for more stability
                    if detected_count >= 3:  # At least 3 pixels must match
                    ...
                    if purple_detected:
                        ...
                        if self.stats['detections'] % 100 == 1:  # Reduced log frequency
                        ...
                        if current_time - last_click_time >= (self.click_cooldown.get() / 1000.0):
                            if self.perform_click_ultra_safe():
                                ...
                                self.stats['clicks'] += 1
                                # Log clicks less frequently
                                if self.stats['clicks'] % 10 == 1:
                                    self.log(f"Click performed! (Total: {self.stats['clicks']})")
                    ...
                    if total_errors % 50 == 1:  # Log every 50th error only
                        self.log(f"Scan error #{total_errors} (continuing anyway): {str(scan_error)[:50]}")
                    
                    # Even with errors, keep running!
                    time.sleep(0.2)  # Longer pause on error for stability
                    
                # Sleep with error handling
                try:
                    sleep_time = max(0.01, self.scan_interval.get() / 1000.0)
